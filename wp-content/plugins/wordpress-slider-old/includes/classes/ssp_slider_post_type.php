...
class SSP_SLIDER_POST_TYPE {
	...
	function options_meta_box( $post ) {
		...
		//default html slider skin
		$skins[] = array(
			'name' => __( 'Default HTML Skin', SLIDER_PLUGIN_PREFIX ),
			'path' => 'default_html',
			'description' => ''
		);
		...
		if ( ! $slider_options )
			...
			$slider_options = wp_parse_args( $slider_options, self::default_options() );
