<?php
function cs_video($atts){
	if(isset($atts['type'])){
		switch($atts['type']){
			case 'html5':
				return cs_video_html5($atts);
				break;
			case 'flash':
				return cs_video_flash($atts);
				break;
			case 'youtube':
				return cs_video_youtube($atts);
				break;
			case 'vimeo':
				return cs_video_vimeo($atts);
				break;
			case 'dailymotion':
				return cs_video_dailymotion($atts);
				break;
		}
	}
	return '';
...
add_shortcode('video', 'cs_video');

function cs_video_html5($atts){
	...
	if (!$height && !$width){
		/*$height = wp_sc_get_option('video','html5_height');
		$width = wp_sc_get_option('video','html5_width');*/
	}
...
function cs_video_flash($atts) {
	...
	if (!$height && $width) $height = intval($width * 9 / 16);
	if (!$height && !$width){
		/*$height = wp_sc_get_option('video','flash_height');
		$width = wp_sc_get_option('video','flash_width');*/
	}
...
function cs_video_vimeo($atts) {
	...
	if (!$height && $width) $height = intval($width * 9 / 16);
	if (!$height && !$width){
		/*$height = wp_sc_get_option('video','vimeo_height');
		$width = wp_sc_get_option('video','vimeo_width');*/
	}
	if($title!='false'){
		$title = 1;
	}else{
	...
	if (!empty($clip_id) && is_numeric($clip_id)){
		return "<div class='video_frame'><iframe src='http://player.vimeo.com/video/$clip_id?title={$title}&amp;byline=0&amp;portrait=0' width='$width' height='$height'></iframe></div>";
	}
}

function cs_video_youtube($atts, $content=null) {
	extract(shortcode_atts(array(
		'clip_id' 	=> '',
		'width' 	=> false,
		'height' 	=> false,
	...
	if (!$height && !$width){
		/*$height = wp_sc_get_option('video','youbube_height');
		$width = wp_sc_get_option('video','youbube_width');*/
	}

	if (!empty($clip_id)){
		return "<div class='video_frame'><iframe src='http://www.youtube.com/embed/$clip_id' width='$width' height='$height' ></iframe></div>";
	}
}

function cs_video_dailymotion($atts, $content=null) {
	...
	if (!$height && $width) $height = intval($width * 9 / 16);
	if (!$height && !$width){
		/*$height = wp_sc_get_option('video','dailymotion_height');
		$width = wp_sc_get_option('video','dailymotion_width');*/
	}

	if (!empty($clip_id)){
		return "<div class='video_frame'><iframe src=http://www.dailymotion.com/embed/video/$clip_id?width=$width&theme=none&foreground=%23F7FFFD&highlight=%23FFC300&background=%23171D1B&start=&animatedTitle=&iframe=1&additionalInfos=0&autoPlay=0&hideInfos=0' width='$width' height='$height' ></iframe></div>";
	}
}